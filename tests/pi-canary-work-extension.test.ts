...
describe("pi canary work extension surface", () => {
	test("registers exactly one tool, two TUI commands, and one assurance renderer", () => {
		...
		expect(Object.keys(commands).sort()).toEqual(["imm-canary-assure", "imm-canary-authorize", "imm-canary-succeed"]);
	...
	test("tool status works in any mode and mutation requires a matching claim", async () => {
		...
		try {
			...
			expect(status.details).not.toHaveProperty("record_revision");
			expect(status.details).not.toHaveProperty("diff_hash");
	...
	test("qa command returns while deterministic verification continues in the background", async () => {
		...
		try {
			...
			expect(ui.notifyCalls.some((call) => /QA 1\/5 \| pi-only-package-surface running \| elapsed 0s \| hard limit 15m/.test(call.text))).toBe(true);
			expect(messages.filter((entry) => entry.options?.deliverAs === "followUp")).toHaveLength(0);
			expect(ui.statusCalls).toEqual([]);
			expect(ui.widgetCalls).toEqual([]);
			releaseQa();
			await command;
	...
	test("native verdict remains advisory until literal-user confirmation", async () => {
		...
		try {
			...
			expect(resultMessage?.message.details.next_action).toBe("request_authorization");
			expect(resultMessage?.message.content).toContain("Call imm_kernel_canary request_authorization now");
			expect(resultMessage?.message.content).not.toMatch(/ask the user|type \/imm-canary-authorize/i);
			await commands["imm-canary-assure"].handler(`${TASK} review`, ctx);
