...
- Spec: `docs/specs/2026-08-13-roadmap-same-phase-continuation.spec.md`
...
- Discovery cache: `plugins/immune-brain/runtime/immune_brain_runtime.ts` (`isApprovedTransitionPhase`, `runApprovedTransition`, lock-time reconstruction); `plugins/immune-brain/runtime/state_ledger.ts` (`buildClosedPlanArchive`, `buildTransitionRecord`, `validateTransitionState`, `collectReviewChangedFiles`); `tests/roadmap-plan-transition-runtime.test.ts` (approved-transition fixtures and no-write matrix); `tests/roadmap-plan-transition-state.test.ts` (archive/transition integrity and linear history); `tests/imm-loop-review-lifecycle-state.test.ts` and `tests/cross-plan-sync-reset.test.ts` (review-scope isolation); `docs/specs/2026-08-13-roadmap-same-phase-continuation.spec.md` (Technical Design authority)
