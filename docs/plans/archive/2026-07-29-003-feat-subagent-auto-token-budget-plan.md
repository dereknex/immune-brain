...
- Origin: User-requested evaluation of Brainstorm and Planner token consumption with subagent authorization defaulting to `auto`; see `docs/specs/archive/subagent-auto-token-budget.spec.md`.
...
- Design Conformance: the implementation must match `docs/specs/archive/subagent-auto-token-budget.spec.md`; a local mismatch is rework, while changing default authorization, budget semantics, persistence boundary, claim scope, or evidence-status meaning is replan.
