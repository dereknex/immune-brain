...
- Spec: docs/specs/archive/imm-loop-review-lifecycle-runtime.spec.md
...
- Discovery cache: plugins/immune-brain/runtime/immune_brain_runtime.ts (MCP tool definitions and command mapping); plugins/immune-brain/.opencode-plugin/index.ts (package tool exposure if helpers are added); plugins/immune-brain/.opencode-plugin/runtime.ts (host command adapter); tests/plugin-package-runtime.test.ts (tool surface regression); docs/specs/archive/run-review-closure-runtime-gate.spec.md (compounder gate acceptance)
- Agent Hint: imm-executor
- Depends on: 3
