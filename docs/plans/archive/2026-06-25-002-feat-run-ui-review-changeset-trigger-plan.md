...
1. **Rollback Resilience**: This change only modifies `docs/specs/archive/run-completion-loop.spec.md`, `plugins/immune-brain/dist/run.md`, `tests/test_skill_contracts.py`, and this plan. Reverting these files returns the system to the previous state with zero side effects on runtime databases or ledgers.
...
- contract surface: `docs/specs/archive/run-completion-loop.spec.md`, `plugins/immune-brain/dist/run.md`, `tests/test_skill_contracts.py`.
...
- Discovery cache: docs/specs/archive/run-completion-loop.spec.md (R2); plugins/immune-brain/dist/run.md (workflow rules); tests/test_skill_contracts.py (unittest)
