...
- Discovery cache: `plugins/immune-brain/runtime/immune_brain_runtime.ts` (`buildSuccessorDecisionProjection`, `runAutoworkCommand`, `buildAutoworkSnapshot`, and post-finish ordering); `tests/roadmap-plan-progression-runtime.test.ts` (pre/post-finish, terminal, priority, revision, and no-write fixtures); `plugins/immune-brain/dist/imm-loop.md` and `plugins/immune-brain/dist/imm-work.md` (host stop contracts); `docs/specs/2026-07-28-roadmap-plan-boundary-successor.spec.md` section 5.7.1 (terminal invariant)
