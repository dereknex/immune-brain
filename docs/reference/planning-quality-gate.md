...
- **risk-triggered exploration**: before freezing a Managed Plan, resolve unknowns that can change Scope, design, or Verification — CI environment, third-party APIs, database behavior, cross-module interfaces — using targeted read-only probes. Ask only the minimum blocking question for unclear framing, then reapply the BASELINE matrix. Eligible work stays on the Direct Path regardless of file count or local verifier count; independently owned domains and unresolved material risk select Managed. Stop probing once Result, Scope, and Verification are concrete.
