# Spec: api-contract-reviewer runtime slice

**任务 ID**: IMM-API-002
**负责人**: Planner
**状态**: Superseded（当前事实：独立 `api-contract-reviewer` skill surface 已删除；API contract 审查通过 `imm-advisory-reviewer` 的 `api_contract` lens 触发）

> Historical note: 本 spec 记录旧的独立 reviewer runtime slice。它不再声明已删除的独立 skill path 是当前 activation host。

## 1. 目标

把 `api-contract-reviewer` 从已闭环的 docs-first contract slice，推进成第二条真正可激活的
conditional-risk reviewer runtime slice。

当前 activation host 已合并到 `imm-advisory-reviewer`，API and public contract changes
通过 `api_contract` lens 审查。它仍然保持 `advisory`、只读、trigger-only，不升级成默认 gate。

## 2. 问题背景

仓库已经完成三层与本任务直接相关的前置工作：

- `api-contract-reviewer` docs-first slice 已把 trigger、manifest-style contract、fallback 和
  focused regression 要求收口到可验证文档层。
- `security-reviewer` 与 project-specific runtime slices 已证明：对 reviewer，先交付最小
  activation host、明确 trigger-only routing，并把真实 runtime 验证留在 manual path，
...
当前缺口在于：`api-contract-reviewer` 仍只存在于 README / spec / plan 的 contract 层，还没有一个
...
- 当前 runtime 不再定义独立本地 skill；使用 `imm-advisory-reviewer` 的 `api_contract` lens。
...
- skill contract 必须显式声明：
  ...
  - dedicated reviewer unavailable 时的 fallback 指向：
    `imm-code-review` plus planner or executor contract notes

### R3. Trigger-only routing

- `api-contract-reviewer` 不得被描述成默认 gate。
- 只有当任务内容、diff 或用户请求明确命中 API route / request schema / response schema /
  serialization / versioning / exported type / public contract 审查时，才应激活这个 reviewer。
...
- [x] 不再存在独立 `api-contract-reviewer` activation host；当前使用 `imm-advisory-reviewer` 的 `api_contract` lens。
- [x] lens contract 明确保持 `advisory`、只读和 trigger-only posture。
- [ ] trigger 面仍收敛在 API route / request schema / response schema / serialization /
      versioning / exported type / public contract 变化。
- [x] unavailable path 明确回退到 `imm-code-review` 与 planner 或 executor 的最小 contract notes，
...
- 不新增 `data-integrity-reviewer`、`reliability-reviewer`、`release-readiness-checker` 或
  `debug-investigator` 的 runtime slice。
- 不把 `api-contract-reviewer` 升级成默认 gate 或所有接口变更的常驻 reviewer。
- 不授予写 plan、改代码、改测试、写 workflow state 的权限。
- 不构建 API diff engine、compatibility platform 或 version-policy framework。

## 6. 依赖项

- 依赖 [api-contract-reviewer.spec.md](docs/specs/api-contract-reviewer.spec.md)
  作为 docs-first contract 与 fallback 基线。
- 依赖 [security-reviewer-runtime.spec.md](docs/specs/security-reviewer-runtime.spec.md)
  与 [prompt-contract-reviewer-runtime.spec.md](docs/specs/prompt-contract-reviewer-runtime.spec.md)
...
1. 在支持项目本地 skill / reviewer activation 的 Codex runtime 中，准备一次明确涉及
   API route、request schema、response schema、serialization、versioning、exported type 或
   public contract 变化的任务。
...
1. 在当前环境没有 dedicated reviewer path，或父流程未加载该 skill surface 时，触发同类
   API / public contract review。
2. 预期行为：
   - 系统不会伪装成已有 dedicated reviewer；
   - fallback 明确回到 `imm-code-review` 与 planner 或 executor 的最小 contract notes；
   - 输出说明这是基础 contract 审查，而不是完整等价替代。
