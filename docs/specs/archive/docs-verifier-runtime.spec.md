# Spec: docs-verifier runtime slice

**任务 ID**: IMM-DOCS-002
**负责人**: Planner
**状态**: Accepted（验收证据：skills/docs-verifier/SKILL.md 提供独立 activation host + advisory-only/trigger-only contract + delegation packet + fallback；tests/test_skill_contracts.py 通过）

## 1. 目标

把 `docs-verifier` 从已闭环的 docs-first contract slice，推进成第一批 subagent activation roadmap
里的下一条可激活 project-specific runtime slice。

首版只要求一个最小、可验证的 activation host：独立的本地 reviewer skill surface，用于
README、用户文档、setup instructions、公开 usage examples 与 behavior-to-docs delta 审查。
...
- `docs-verifier` docs-first slice 已把 trigger、manifest-style contract、fallback 和
  focused regression 要求收口到可验证文档层。
- `prompt-contract-reviewer` 与 `ai-eval-planner` runtime slices 已证明：对 project-specific
  reviewer，先交付最小 activation host、明确 trigger-only routing，并把真实 runtime 验证留在
  manual path，比直接做 shared runtime 更稳。
- `first-subagent-batch` activation roadmap 已把当前阶段锁定为“逐个推进到可激活使用”，并在
  `ai-eval-planner` 之后默认顺推 `docs-verifier`，而不是直接转向 conditional-risk reviewers
  或平台工作。

当前缺口在于：`docs-verifier` 仍只存在于 README / spec / plan 的 contract 层，还没有一个
...
- skill contract 必须显式声明：
  ...
  - dedicated reviewer unavailable 时的 fallback 指向：
    `executor` manual docs check or `imm-code-review` baseline docs consistency check

### R3. Trigger-only routing

- `docs-verifier` 不得被描述成默认 gate。
- 只有当任务内容、diff 或用户请求明确命中 README / user docs / setup instructions /
  usage examples / behavior-to-docs delta 审查时，才应激活这个 reviewer。
...
- [ ] 存在一个独立、可引用的 `docs-verifier` activation host，而不只是文档中的 roster 名称。
- [ ] host contract 明确保持 `advisory`、只读和 trigger-only posture。
- [ ] trigger 面仍收敛在 README / user docs / setup instructions / usage examples /
      behavior-to-docs delta 变化。
- [ ] unavailable path 明确回退到 `executor` manual docs check 或 `imm-code-review`，
...
- 不新增 `security-reviewer`、`api-contract-reviewer`、`release-readiness-checker` 或
  `debug-investigator` 的 runtime slice。
- 不把 `docs-verifier` 升级成默认 gate 或所有文档变更的常驻 reviewer。
- 不授予写 plan、改代码、改测试、写 workflow state 的权限。
- 不构建 docs pipeline、public docs index、release-notes framework 或自动发布系统。

## 6. 依赖项

- 依赖 [docs-verifier.spec.md](docs/specs/docs-verifier.spec.md)
...
1. 在支持项目本地 skill / reviewer activation 的 Codex runtime 中，准备一次明确涉及
   README、用户文档、setup instructions、usage examples 或 behavior-to-docs delta 的任务。
...
2. 预期行为：
   ...
   - fallback 明确回到 `executor` manual docs check 或 `imm-code-review`；
   - 输出说明这是基础文档核对，而不是完整等价替代。
