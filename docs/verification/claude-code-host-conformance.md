...
- CLI envelope: `claude --version` → `2.1.236 (Claude Code)`; `claude plugin validate --strict plugins/immune-brain` → `Validation passed` (exit 0); plugin 3.2.1 from `.claude-plugin/plugin.json`, MCP server loads `dist/claude/mcp-server.mjs`. The 2026-09-03 live session captures below were recorded against plugin 2.8.3; contract-fixture scenarios were re-verified against package 3.2.1.
