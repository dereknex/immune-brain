...
reusability: medium
key_files:
  - docs/specs/archive/architecture-improvement-wave-3.spec.md
...
**证据**: `docs/specs/archive/architecture-improvement-wave-3.spec.md` §4 Non-Goals —「不改变 `install-local.sh` 的 copy-based 安装方式或 CLI wrapper 行为」；U004 验收仅添加 `pip install -e .imm/` 供本仓库测试，未修改 `install-local.sh`。
