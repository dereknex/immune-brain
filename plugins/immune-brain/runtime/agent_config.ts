...
export type WorkflowReasoningEffort =
	...
	| "xhigh"
	| "max";
export type WorkflowVerbosity = "low" | "medium" | "high";

export interface WorkflowModelOptions {
	reasoning_effort?: WorkflowReasoningEffort;
	verbosity?: WorkflowVerbosity;
}

export interface AdvisoryDispatchConfig {
	dev_insights?: {
		enabled?: boolean;
		inbox_path?: string;
	};
	subagent_activation?: { default?: string; hosts?: Record<string, string> };
	workflow?: { model_preset?: string };
	workflow_models?: Record<string, string | string[]>;
	workflow_model_options?: Record<string, WorkflowModelOptions>;
	subagent_models?: Record<string, unknown> & {
		fast?: string;
		mid?: string;
		strong?: string;
		local?: string;
		inherit?: string;
		lens_overrides?: Record<string, string>;
	};
}
...
export function readImmuneBrainConfig(
	...
	for (const path of paths) {
		...
		config = mergeConfig(
			config,
			parseImmuneBrainConfigToml(readFileSync(resolved, "utf8")) as Record<string, any>,
		);
