...
When the Kernel projection reports an active/draining backend claim, the owned task is routed to `imm-canary-work` (Pi lifecycle extension); `imm-work` must not mutate or mirror a Kernel-owned task. After fresh executor evidence, the parent calls `imm_kernel_canary` `advance_assurance` and relies on visible background state plus push follow-up; it does not manually sequence QA/Review slash commands or poll a child. A terminal tombstone alone never blocks ordinary v3 routing for a different task.
