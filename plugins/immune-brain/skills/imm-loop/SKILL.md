...
for Kernel ownership. Do not discover or load a Pi Skill for these roles. The
public Skills remain available as rollback shims during this additive
migration. Shim removal owner: the Immune-Brain maintainer. Cleanup milestone:
the next minor release after Issue #9's three-entry public surface contract and
the Issue #6/#7 Loop parity tests pass; until then they are the documented
rollback path.
...
The loop always enters through this action builder: the action's `next` authority is `executor`, `test-fixer`, `pr-fix`, `imm_kernel_canary`, or `imm-planner`.
